...
1. **Normalize** (LLM call 1, skipped on a cache hit): product in; category, canonical name, similar products and search queries out, validated against `proxy/schemas/normalize.json`, retried once, then refused. Blocked brand names are stripped from every field. The words best, top, review, reviews and vs are removed from online queries unless the shopper typed them, a local query ending in shop or shops ends in store or stores instead, " store" is appended to one that ends in none of those words or outfitters, and duplicate local queries are sent once. Then the arrays are truncated (6 similar products, 3 online and 2 local queries). The raw model output is cached in Workers KV (`NORMALIZE_CACHE`, 30 days) under a SHA-256 hash of the prompt and model list, built from the lowercased product; a cached value is used only if it passes the schema and the scrub, so blocklist changes still apply; a reading with no local query is not cached. The key uses the lowercased product while the model sees it as typed, so the first search for a product sets its reading for every casing. Without the binding every search calls the model.
2. **Fetch** (Brave): web search per online query, `place_search` per local query, at most 5 calls; the client refuses any call past 6. Results on a negative-source domain (regulators, courts, news) are dropped here: they are evidence, not shops. A place whose store type (`icon_category`) is restaurant or amusement park is dropped too; any other store type becomes the start of the place's snippet, so the classifier sees more than a name.
...
6. **Enrich**: curated certifications and findings joined by registrable domain, then LLM call 2 (skipped when no unflagged candidate survives). The model sees at most 24 online and 20 local candidates (every place result the two local queries can return), each with an id, and the product. The response's `usage.unclassified_shown` counts shown results the model did not judge. Code accepts a model-suggested signal only when it cites a fetched page ([ranking.md](ranking.md)). The same call classifies each candidate by id: a site type (retailer, marketplace, editorial, manufacturer_no_cart, service, other) and whether it sells the product (yes, maybe, no).
...
10. **Respond**: local shops are first split by distance ([ranking.md](ranking.md#distance-groups): nearby within 10 mi, or 30 mi when the request's optional `ruca` code is 4 or more; up to 3 more under `local_farther` out to 100 mi; beyond that dropped as `too_far`), then top 10 per section (near you, online), with at most 2 branches of one registrable domain near you, so a chain's many nearby branches cannot push out independent shops, with weights, components, sources, and usage (Brave calls, tokens, estimated cost). The response also carries the exact queries sent to Brave and a `dropped` list (kind, registrable domain and reason for each candidate the place-category rule or the drop step removed, or the branch cap or top-10 cut left out, at most 60, checked against the blocklist again), so an evaluation can tell a shop the filters removed from one the search never found.
...
**Infrastructure** (`infra/`, OpenTofu, Cloudflare provider 5.x). Every Worker and infrastructure change is deployed with `infra/deploy.sh`, never with hand-typed `tofu` commands. Secrets and account values come only from the key files named in a gitignored config (template `infra/deploy.local.env.example`); nothing is committed in a `.tfvars` file. The Cloudflare API token needs two account permissions: Workers Scripts, Edit and Workers KV Storage, Edit (without the second, creating the cache namespace fails with 401 Authentication error). The script checks the tree is committed and pushed, runs the proxy tests and build, plans, and applies only a new Worker bundle, refusing any other change (`infra/plan-guard.mjs`). For a change the guard refuses (a new resource, a binding, a limit), the operator runs `infra/deploy.sh --operator`: the same steps, but the full plan is shown (keys appear as `(sensitive value)`) and applied only after the operator types the commit hash; it needs a terminal, so an agent cannot answer it. Either way the script confirms a fresh plan is empty and the Worker answers a CORS preflight. When to run it: [.claude/skills/deploy/SKILL.md](../.claude/skills/deploy/SKILL.md).

This creates the Worker (`cloudflare_workers_script`, with the two keys as secret bindings, `ALLOWED_ORIGIN`, `SITE_NAME` and `SITE_URL` as plain bindings, two rate-limit bindings, and the `NORMALIZE_CACHE` binding to a Workers KV namespace, `cloudflare_workers_kv_namespace`) and enables its `workers.dev` address (`cloudflare_workers_script_subdomain`). There is no route resource until a custom domain exists. Rate limits default to 30 per minute per client and 60 per minute globally (`rate_limit_per_minute`, `global_limit_per_minute`).
