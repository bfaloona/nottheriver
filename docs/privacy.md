...
| Cloudflare Workers KV (normalize cache) | The model's reading of each product searched: its category, generic name, similar products and search wording. Stored under a SHA-256 hash of the product text (lowercased, spaces collapsed); no location or IP address. Each entry's expiry is kept, so the time of the first search for a product can be worked out from it. A hash of short text can be reversed by guessing the text, and the stored reading usually names the product anyway, so treat this as a list of products people searched for; whatever someone types as a product can be echoed in it | 30 days from the first search for that product, then deleted by Workers KV. Live once the operator deploys the KV namespace (`infra/deploy.sh --operator`); until then nothing is cached |
