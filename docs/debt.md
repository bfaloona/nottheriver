...
| The Cloudflare rate-limiting bindings did not throttle in a live test on 2026-09-23 (135 requests from one client in about a minute, no 429; cause not confirmed). An in-memory limiter now always applies as well and returned `429` live over one reused connection, but it counts per isolate, so separate connections escape it; the provider spend caps remain the real backstop | A Durable Object counter shared by all isolates |
...
| Map pins show a shop's rank and name on hover only and are kept out of the tab order (the list is the keyboard and screen-reader equivalent); a pin does not scroll to or highlight its result; shops at one address or block stack and hide each other's numbers (seen live: three Pike Place shops); the tiles are not dimmed in dark mode | Make each pin a working link to its result and back; spread or group stacked pins; a dark tile style |
