...
| Map pins show a shop's rank and name on hover only and are kept out of the tab order (the list is the keyboard and screen-reader equivalent); a pin click leads to its result, but a result does not highlight its pin; shops at one address or block stack and hide each other's numbers (seen live: three Pike Place shops); the tiles are not dimmed in dark mode | Highlight a result's pin on hover or focus; spread or group stacked pins; a dark tile style |
