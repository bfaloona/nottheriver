...
| When "tier is provisional" comes off (pass 3) | Lead | Removed once every applicable news search ran: 2 for a US retailer with no named parent, 3 otherwise | Removed from all 38 pass-3 retailers |
| Parent company actions | Operator (OC32) | A parent company's actions count against the retailer, including actions from before it bought the retailer. Replaces the N4 agent's narrower rule (only actions about the retailer's own business) | Depop to `caution` (eBay's 2024 fine). Naver's Korean fines (Poshmark) still don't count: no accepted source |
| Proposition 65 | Operator (OC33); kind and counting by the lead | Settlements and judgments recorded on oag.ca.gov count, private enforcers included; a 60-day notice alone doesn't. Kind `environmental` (the law is run by CalEPA's OEHHA and is about toxic-chemical exposure). One row per settlement agreement | Etsy and Bob's Red Mill to `caution`; Equal Exchange, Grove and Uncommon Goods to `acceptable`; Patagonia to `caution` after OC37 (sister-company settlements count) |
| World of Books B Corp | Operator (OC36, and a follow-up yes) | The operator opened B Lab's page; the certification is also added to `data/certifications.json`, the live list | World of Books `recommended`; live after merge and deploy |
| New accepted sources | Operator (OC33, OC34) | fda.gov and ca.gov added to `data/negative-sources.json` and `.md`, which the live Worker also reads | Public Goods to `caution`; Overstock's Air Resources Board settlement counts (no tier change). Live after merge and `infra/deploy.sh` |
| Overstock's court concern source (pass 3) | Lead | The CourtListener opinion page is the `source`; its text didn't render to the fetch tool, so the quotes come from CourtListener's search API for the same opinion (pass 2 used that API too) | Overstock goes to `caution` |
...
- **Concern searches were incomplete until pass 3.** The run's 200 web searches ran out during the retailer stage. Later checks used regulator and court pages fetched directly, mostly OSHA, NLRB and CourtListener. Each `raw/blocked-retailers-R*.md` file lists what was not searched. All 50 rated retailer files carried a "tier is provisional" line. Verifiers added it to 47; the lead added it to the other three (Eileen Fisher, Kotn, Made Trade), which were also searched after the budget ran out ([raw/blocked-retailers-R10.md](raw/blocked-retailers-R10.md)). Pass 3 (below) removed it from the 38 recommended and acceptable retailers; the 12 caution retailers keep it, since a new concern can't change their tier.
- **A second concern pass found nothing new.** Later on 2026-09-25, still without web search, four agents rechecked the 38 recommended and acceptable retailers against FTC case titles, CourtListener dockets since 2016 where a government body is a party, and ProPublica. They added no concerns and changed no tiers ([raw/concern-pass-2-brief.md](raw/concern-pass-2-brief.md), `raw/concerns2-C1.md` to `C4.md`). The 12 caution retailers were skipped because a new concern can't change their tier. It had no general news search.
- **A third pass searched the news.** In a later session with a larger search budget, four agents ran 2 or 3 web searches per retailer (lawsuits and fines; named US agencies; the parent company or, for a non-US retailer, its home regulator) on the same 38 retailers ([raw/news-search-brief.md](raw/news-search-brief.md), `raw/news-N1.md` to `N4.md`). Two accepted concerns were added: a 2016 CPSC civil penalty against Best Buy and a 2017 California Court of Appeal ruling upholding civil penalties against Overstock. Both move from `acceptable` to `caution`, since an accepted concern deducts whatever its age (the 5-year window only gates `recommended`). No `recommended` retailer changed. The pass left actions from FDA, the California Air Resources Board and oag.ca.gov Proposition 65 records as uncounted notes; the operator's later rulings made them count (see the decisions table). Canadian regulators remain notes.
- **Pass 1's OSHA and NLRB searches used pages that robots.txt disallows** (`osha.gov/ords/`, `nlrb.gov/search/`). This broke the run's own access rule. Every OSHA concern came from those pages. The operator decided on 2026-09-25 to keep them, and all 18 were then confirmed through DOL's open-data API, which robots.txt allows: each inspection matches on establishment, citation issue date, close date and penalty ([raw/osha-dol-match.md](raw/osha-dol-match.md), OC31). No NLRB concern came from the disallowed search.
...
- **Foreign regulators count only as notes.** Pass 3 searched the home regulator of each non-US retailer (Tentree, Kotn, Kobo, World of Books, Shop, Depop), but those regulators' pages aren't on the accepted list ([raw/verify-R10.md](raw/verify-R10.md) first noted the gap).
...
| OC5 | Confirm the B Corp claims that have no `data/certifications.json` row: Back Market, Better World Books, EarthHero, Kotn, Libro.fm, Love Grown, Thrive Market, Bookshop.org, Tentree | `raw/blocked-retailers-R1.md`, `-R2`, `-R3`, `-R5`, `-R6`, `-R7`, `-R10` | +0.25 ethics each. The first six go from `acceptable` to `recommended`. Thrive Market goes from `caution` to `acceptable`. Bookshop.org and Tentree are already `recommended`. |
...
| OC29 | Confirm that Plaid now sells Arteza (K4 used plaidonline.com) and that Hive rebranded as Love Grown (pass 3 found a 2023 press release saying Hive Brands *acquired* Love Grown; `parent` left unknown) | [raw/calibration.md](raw/calibration.md); [raw/verify-R6.md](raw/verify-R6.md); [raw/news-N3.md](raw/news-N3.md) | Tally names and domains only |
...
| OC31 | Concerns found through pages that robots.txt disallows (`osha.gov/ords/`, `nlrb.gov/search/`), which pass 1 used. **Decided: keep** (operator, 2026-09-25). **Done:** all 18 OSHA concerns confirmed in DOL's open data, a robots-allowed channel (issue date, close date and penalty match); each retailer file's Sources names its DOL record, and the osha.gov links stay as the `source` a person can open ([raw/osha-dol-match.md](raw/osha-dol-match.md)) | [raw/run-log.md](raw/run-log.md) | Keeping them changes nothing. Dropping them removes 18 concerns from 9 retailers and changes 8 tiers: Avocado and Bob's Red Mill to recommended; Azure Standard, Barnes & Noble, B&H, Chewy, ThredUp and Thrive Market to acceptable; Costco stays caution |
| OC32 | Decide whether a parent company's actions count against a retailer. Pass 3 counted them only when they concern the retailer's own business (N4), and N2 didn't carry eBay's concerns to Depop. No Rakuten Group search ran for Kobo **Decided: yes, parent actions count** (operator, 2026-09-25). Depop took eBay's concern; a Rakuten Group search for Kobo found nothing on an accepted source. | [raw/news-N2.md](raw/news-N2.md); [raw/news-N4.md](raw/news-N4.md); [raw/news-N3.md](raw/news-N3.md) | If they count: Depop goes from `acceptable` to `caution` (eBay's 2024 concern); others unknown until parent searches run |
| OC33 | Decide whether oag.ca.gov Proposition 65 notices join the accepted sources, and whether private-enforcer settlements with a court judgment count (Equal Exchange 2016, Patagonia Provisions 2022, Uncommon Goods 2006 with judgment, Grove 2025, Bob's Red Mill consent judgment) **Decided: yes** (operator, 2026-09-25). Added as `ca.gov` in `data/negative-sources.json`, since sources match on registrable domain. All 34 non-caution retailers were then checked ([raw/prop65-P1.md](raw/prop65-P1.md)). | [raw/news-N1.md](raw/news-N1.md); [raw/news-N2.md](raw/news-N2.md) | If all count: Equal Exchange, Patagonia, Uncommon Goods and Grove go from `recommended` to `acceptable`; Bob's Red Mill goes from `acceptable` to `caution` |
| OC34 | Decide whether fda.gov joins the accepted sources **Decided: yes** (operator, 2026-09-25). Public Goods moved to `caution`. | [raw/news-N4.md](raw/news-N4.md) | Public Goods goes from `acceptable` to `caution` (2022 FDA warning letter) |
| OC35 | Decide whether a voluntary CPSC recall that states a regulatory violation, with no penalty, is a finding **Decided: no** (operator, 2026-09-25). No row. | [raw/news-N2.md](raw/news-N2.md) | No tier change (Avocado's ethics is already 0) |
| OC36 | Open the B Lab directory page for World of Books Group (https://www.bcorporation.net/en-us/find-a-b-corp/company/world-of-books-group/, 403 to agents) **Decided: certified** (operator checked the page, 2026-09-25). World of Books moved to `recommended`. | [raw/news-N4.md](raw/news-N4.md) | Adds to OC5: World of Books goes from `acceptable` to `recommended` if certified |
| OC37 | Decide whether Proposition 65 settlements naming only Patagonia Provisions (2022, 2023, 2025) count against Patagonia. Provisions sits beside Patagonia, Inc. under the same parent, Patagonia Works (all three were defendants in the counted 2021 settlement), so it's a sibling company, not a parent **Decided: yes** (operator, 2026-09-25). Patagonia moved to `caution` (environment 0, total 0.75), not `acceptable` as first estimated: all 3 rows are environmental. | [raw/prop65-P1.md](raw/prop65-P1.md) | If they count: Patagonia goes from `recommended` to `acceptable` (concerns within 5 years) |
