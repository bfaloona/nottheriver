...
- **Pass 1's OSHA and NLRB searches used pages that robots.txt disallows** (`osha.gov/ords/`, `nlrb.gov/search/`). This breaks the run's own access rule. Every OSHA concern in the retailer files comes from those pages. They are real agency records, and nothing has been removed pending an operator decision (OC31).
